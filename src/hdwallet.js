...
const trivechainTestnetNetwork = {
  magicPrefix: '\x18Trivechain Signed Message:\n',
  bip32: {
    public: 0x043587cf,
    private: 0x04358394
  },
  ...
  dustThreshold: 546,
  feePerKb: 10000,
  estimateFee: estimateFee('trivechaintestnet')
}

const trivechainNetwork = {
  magicPrefix: '\x18Trivechain Signed Message:\n',
  bip32: {
    public: 0x0488b21e,
    private: 0x0488ade4
  ...
  estimateFee: estimateFee('trivechain')
}
...
HDWallet.createNewKey = function (network, pass, progressCallback) {
  ...
  network = (network === 'testnet' ? trivechainTestnetNetwork : trivechainNetwork)
  ...
  // coin_type'
  node = node.deriveHardened(network === trivechainNetwork ? 0 : 1)
  // account'
...
HDWallet.getInputAddresses = function (txHex, network) {
  network = network || trivechainNetwork
  var addresses = []
  var tx
  try {
...
HDWallet.prototype.getKeyPrefix = function () {
  ...
  var network = (self.network === trivechainNetwork) ? 'mainnet' : 'testnet'
...
HDWallet.prototype.registerAddress = function (address, accountIndex, addressIndex, change) {
  ...
  var coinType = self.network === trivechainNetwork ? 0 : 1
  change = (change) ? 1 : 0
...
HDWallet.prototype.deriveAccount = function (accountIndex) {
  ...
  if (this.preAccountNode) {
    ...
    node = node.deriveHardened(44)
    // coin_type'
    node = node.deriveHardened(this.network === trivechainNetwork ? 0 : 1)
